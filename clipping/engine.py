...
def _make_ydl_progress_hook(progress_callback=None):
    """Build a yt-dlp progress hook that prints a console bar and,
    if given, forwards structured progress to *progress_callback*.

    yt-dlp downloads the video and audio streams separately, so this hook
    is called once per stream; the newline on "finished" keeps each bar
    on its own line.
    """
    ...
    def _hook(d: dict) -> None:
        ...
        if status == "downloading":
            ...
            if total:
                ...
                # Unknown size (live/streamed manifest) — show bytes + speed only.
                print(
            ...
            print(flush=True)  # close out this stream's bar line
...
def transcribe_video(
    ...
    # Batched inference: VAD-segments the audio and runs multiple speech
    # regions through this ONE loaded model instance concurrently, instead of
    # one long sequential decode. Do not spawn separate WhisperModel processes
    # in parallel instead — each would load its own full model copy (~3GB for
    # large-v3) and their internal thread pools would oversubscribe the same
    # CPU cores, which is slower overall, not faster (confirmed via testing:
    # this is what caused long videos to appear to hang).
    model = WhisperModel(model_size, device=device, compute_type=compute_type)
    batched_model = BatchedInferencePipeline(model=model)

    print("      ⏳ Mendekode audio & mengekstrak fitur (belum ada output)...", flush=True)
    # initial_prompt is intentionally omitted -- known to reduce accuracy in
    # batched mode (community-confirmed, faster-whisper discussion #1057).
    segments, info = batched_model.transcribe(
        video_path, beam_size=5, word_timestamps=True, batch_size=16
    )
